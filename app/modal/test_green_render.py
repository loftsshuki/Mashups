...
from green_render import render_candidates, ROLES, ARRANGEMENTS, number, tempo_ratio
...
class RendererTests(unittest.TestCase):
    ...
    def test_rejects_extreme_tempo_warp(self):
        self.assertAlmostEqual(tempo_ratio(124,128),128/124)
        for source,target in ((90,128),(160,110)):
            with self.assertRaises(ValueError): tempo_ratio(source,target)
    ...
    def test_real_ffmpeg_three_candidates(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            for row in result:
                ...
                self.assertTrue(Path(row["path"]).is_file());self.assertTrue(math.isfinite(row["metrics"]["integratedLufs"]))
                self.assertLessEqual(row["metrics"]["truePeakDb"],-1);self.assertNotEqual(row["qualityStatus"],"passed")
                self.assertEqual(row["metrics"]["musicalQuality"],"not_evaluated");self.assertEqual(row["metrics"]["renderer"],"green-ffmpeg-v2")
                self.assertTrue(row["metrics"]["startAlignment"]["left"]["nearBeat"]);self.assertIn("tempoRatios",row["metrics"])
