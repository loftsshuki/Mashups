...
function SystemMark({ number, icon: Icon, label, inverse = false }: { number: string; icon: typeof Rocket; label: string; inverse?: boolean }) { return <div data-testid="domination-system" className={`flex items-center gap-3 font-mono text-[10px] font-semibold uppercase tracking-[0.15em] ${inverse ? "text-current" : "text-muted-foreground"}`}><span className={`grid size-8 place-items-center border ${inverse ? "border-current" : "border-foreground"}`}>{number}</span><Icon className="size-4" /><span>{label}</span></div> }
