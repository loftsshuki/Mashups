...
export function GreenStudioWorkspace({ initialLeft, initialRight, projectId, usePreviousDraft }: { initialLeft: string; initialRight: string; projectId?: string; usePreviousDraft: boolean }) {
    ...
    async function restore() {
      try {
        ...
        if (projectId) {
          ...
          if (project.sources.kind !== "prototype") throw new Error("This project needs the full catalog renderer, which is not connected yet.")
          ...
          if (!cancelled) {
            setDraft({ ...newLocalDraft(project.sources.leftId, project.sources.rightId), input, cloudFingerprint: recipeFingerprint(input), renderKey: renderFingerprint(input) })
            setNotice("Recipe loaded from your account. Generate the audio to hear it on this device.")
          }
          return
        }
        ...
        if (cancelled) return
        setNotice(error instanceof Error ? error.message : "Draft recovery is unavailable.")
        if (projectId || usePreviousDraft) setFailed(true)
        else setDraft(newLocalDraft(initialLeft, initialRight))
      }
    }
    void restore()
  ...
  return <div className="mx-auto flex min-h-[65vh] max-w-xl flex-col justify-center gap-5 px-6 pt-28">
    {failed ? <><h1 className="display-type text-4xl">Couldn’t open this draft.</h1><p role="alert">{notice}</p><p className="text-sm text-muted-foreground">Starting a new draft will keep any existing draft record on this device.</p><Button onClick={() => { setDraft(newLocalDraft(initialLeft, initialRight)); setNotice("Started a new device draft.") }}>Start a new draft</Button></> : <p role="status" className="flex items-center gap-3"><Loader2 className="size-5 animate-spin" />Opening your studio…</p>}
  </div>
}
