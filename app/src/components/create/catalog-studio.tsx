...
export function CatalogStudio({projectId:initialId,parentId,fresh=false}:{projectId?:string;parentId?:string;fresh?:boolean}) {
 ...
 return <main className="mx-auto min-h-screen max-w-6xl px-5 pb-24 pt-28">
  ...
  {project && <>
   ...
   {project.status==="draft" && <section className="mt-6 border border-foreground bg-card p-6"><h2 className="text-2xl font-semibold">Prepare the render</h2><p className="mt-3 text-sm text-muted-foreground">Choose excerpt start points and tempo. The renderer refuses aggressive tempo warps, measures output loudness and peak, and still requires human listening review. Start points are not claimed to be detected downbeats.</p><div className="mt-5 grid gap-4 sm:grid-cols-3">{Object.entries(recipe).map(([key,value])=><label key={key} className="text-sm">{({targetBpm:"Target BPM",durationSeconds:"Preview seconds",leftStartSeconds:"A start (seconds)",rightStartSeconds:"B start (seconds)",leftSemitones:"A pitch (semitones)",rightSemitones:"B pitch (semitones)"} as Record<string,string>)[key]}<input type="number" className="mt-2 block min-h-12 w-full border bg-background p-3" value={value} onChange={event=>{renderId.current=null;setRecipe(old=>({...old,[key]:Number(event.target.value)}))}}/></label>)}</div><button className={`${button} mt-6`} disabled={busy} onClick={()=>{renderId.current??=crypto.randomUUID();void act("render",{recipe,requestId:renderId.current})}}>Generate three real arrangements</button></section>}
   {project.status==="rendering" && <p role="status" className="mt-6 border p-6">{project.job?.status==="queued"?"Waiting for the processor.":"Rendering your arrangements."}{project.job?.attemptCount?` Attempt ${project.job.attemptCount}${project.job.maxAttempts?` of ${project.job.maxAttempts}`:""}.`:""} Status updates automatically. No progress percentage is estimated.</p>}\n   {project.status==="draft" && project.job?.status==="failed" && <div role="alert" className="mt-6 border border-destructive bg-destructive/10 p-5"><p className="font-semibold">The last render did not finish.</p><p className="mt-2 text-sm">{project.job.errorCode??"PROCESSING_FAILED"}{project.job.errorMessage?` · ${project.job.errorMessage}`:""}</p><p className="mt-2 text-sm text-muted-foreground">Your saved project is intact. Review the recipe and generate again to create a fresh durable job.</p></div>}
   <div ref={audioRack} className="mt-8 grid gap-4 lg:grid-cols-3">{project.candidates.map(candidate=><article key={candidate.id} className="border border-foreground bg-card p-5"><h2 className="text-xl font-semibold">{candidate.arrangement}</h2><audio className="mt-5 w-full" controls preload="none" src={candidate.audioPath} onPlay={event=>audioRack.current?.querySelectorAll("audio").forEach(audio=>{if(audio!==event.currentTarget)audio.pause()})}/><p className="mt-3 text-sm">{candidate.durationSeconds.toFixed(1)} seconds · {candidate.qualityStatus.replaceAll("_"," ")}</p><p className="mt-2 text-xs">{candidate.keepReviews} independent keep reviews</p><div className="mt-3 grid grid-cols-2 gap-2 text-xs text-muted-foreground"><span>LUFS {candidate.integratedLufs?.toFixed(1)??"pending"}</span><span>Peak {candidate.truePeakDb?.toFixed(1)??"pending"} dBTP</span><span>{candidate.technicalPass===true?"Technical checks passed":candidate.technicalPass===false?"Technical checks failed":"Technical checks pending"}</span><span>{candidate.renderer??"Renderer provenance pending"}</span></div><button className={`${button} mt-5 w-full`} disabled={busy||project.status!=="ready"||candidate.qualityStatus==="failed"} onClick={()=>void act("select",{candidateId:candidate.id})}>{project.selectedCandidateId===candidate.id?"Kept":"Keep this version"}</button></article>)}</div>
