...
import { ArrowDownToLine, ArrowRight, Building2, Check, Copy, Loader2, Plus, Radar, Users } from "lucide-react"
...
export function OutreachCrm({ workspace, demo }: { workspace: OutreachWorkspace; demo: boolean }) {
  ...
  async function advance(prospect: OutreachProspect) {
    ...
    try {
      ...
      setProspects((current) => current.map((item) => item.id === prospect.id ? { ...item, stage: next, nextAction: suggestedAction(next) } : item))
  ...
  async function add(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      setProspects((current) => [{ id: data.prospectId ?? crypto.randomUUID(), entityName, entityType: body.entityType as ProspectType, contactName: body.contactName, contactEmail: body.contactEmail, city: body.city, countryCode: body.countryCode, genres: body.genres, catalogSignal: body.catalogSignal, priority: body.priority, stage: "research", nextAction: body.nextAction, dueAt: null, sourceUrl: null, notes: null }, ...current])
  ...
  return <main className="pilot-shell pb-28 pt-[68px]">
    <div className="border-b border-foreground bg-foreground px-4 py-2 text-background"><div className="mx-auto flex max-w-[1700px] justify-between font-mono text-[10px] font-bold uppercase tracking-[0.16em]"><span>Acquisition desk / Independent electronic</span><span>{demo ? "Fictional pipeline / simulation" : "Live pipeline"}</span></div></div>
    <section className="mx-auto max-w-[1700px] px-4 sm:px-6 lg:px-8">
      ...
      <div className="grid border-x border-b border-foreground xl:grid-cols-[1fr_22rem]">
        <section className="min-w-0 p-4 sm:p-7 xl:border-r xl:border-foreground"><div className="flex flex-wrap items-end justify-between gap-4 border-b border-foreground pb-4"><div><p className="mono-label text-muted-foreground">Working pipeline</p><h2 className="display-type mt-2 text-4xl">Next best action</h2></div><div className="flex gap-2"><Button variant="outline" asChild><a href={`/api/pilot/outreach/export${demo ? "?demo=1" : ""}`} data-testid="export-outreach"><ArrowDownToLine />Export CSV</a></Button><Button onClick={() => setShowAdd((value) => !value)}><Plus />Add target</Button></div></div>
...
function MiniField({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) { return <div className={className}><Label className="mb-2 font-mono text-[9px] font-bold uppercase tracking-widest">{label}</Label>{children}</div> }
