...
import { ArrowRight, Check, ClipboardCheck, Gauge, Loader2, LockKeyhole, ShieldCheck, SlidersHorizontal } from "lucide-react"
...
export function OperatorConsole({ workspace, demo }: { workspace: PilotWorkspace; demo: boolean }) {
  ...
  const seed = workspace.operatorConfig ?? defaults(intake?.id ?? "")
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body: PilotOperatorConfig = {
      intakeId, status: "draft",
      guaranteedLaunch: { creatorFloor: int(form, "creatorFloor"), qualifiedPostFloor: int(form, "qualifiedPostFloor"), genomeFloor: int(form, "genomeFloor"), reportDueDays: int(form, "reportDueDays") },
      growthLicense: { territories: split(form, "territories"), termDays: int(form, "termDays"), paidMediaCapCents: cents(form, "paidMediaCap"), takedownSlaHours: int(form, "takedownSlaHours") },
      exclusivity: { type: str(form, "exclusivityType") as PilotOperatorConfig["exclusivity"]["type"], days: int(form, "exclusivityDays"), slots: int(form, "campaignSlots") },
      artistParticipation: { reactionCount: int(form, "reactionCount"), repostCount: int(form, "repostCount"), liveSessionCount: int(form, "liveSessionCount") }, partnerRails,
      measurementPlan: { metric: str(form, "metric") as PilotOperatorConfig["measurementPlan"]["metric"], controlMethod: str(form, "controlMethod"), minimumCohort: int(form, "minimumCohort") },
      commercialOffer: { tier: str(form, "tier") as PilotOperatorConfig["commercialOffer"]["tier"], creatorRewardCents: cents(form, "creatorReward"), platformFeeCents: cents(form, "platformFee") },
      protectionTerms: { enabled: protection, serviceCreditCapCents: cents(form, "serviceCreditCap"), excludes: split(form, "excludes") },
    }
    try {
      ...
      setFeedback({ text: `Operator setup ${data.status}. Readiness is ${data.readiness}%.` })
  ...
  return <main className="pilot-shell pb-28 pt-[68px]">
    <div className="border-b border-foreground bg-primary px-4 py-2 text-primary-foreground"><div className="mx-auto flex max-w-[1600px] justify-between font-mono text-[10px] font-bold uppercase tracking-[0.16em]"><span>Operator desk / {intake.campaignName}</span><span>{demo ? "Simulation" : "Live workspace"}</span></div></div>
    <section className="mx-auto max-w-[1600px] px-4 sm:px-6 lg:px-8">
      ...
      <form onSubmit={submit} className="border-x border-b border-foreground" data-testid="operator-form">
        ...
        <div className="grid gap-5 p-5 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center"><div>{feedback ? <p role={feedback.error ? "alert" : "status"} data-testid="operator-result" className={`border p-3 text-sm ${feedback.error ? "border-destructive text-destructive" : "border-foreground bg-secondary"}`}>{feedback.text}</p> : <p className="text-sm text-muted-foreground">Saving marks configured operating gates ready. Rights authority remains independently verified.</p>}</div><div className="flex flex-wrap gap-3"><Button variant="outline" asChild><Link href={`/outreach${demo ? "?demo=1" : ""}`}>Open outreach</Link></Button><Button type="submit" disabled={pending || !partnerRails.length} data-testid="save-operator">{pending ? <Loader2 className="animate-spin" /> : <ShieldCheck />}Lock operator brief<ArrowRight /></Button></div></div>
      </form>
    </section>
  </main>
}
...
function NumberField({ name, label, value, min, dark }: { name: string; label: string; value: number; min: number; dark?: boolean }) { return <div><Label className={`operator-label ${dark ? "text-background/60" : ""}`}>{label}</Label><Input name={name} type="number" min={min} required defaultValue={value} className={dark ? "rounded-none border-background/40 bg-background/10 text-background" : "rounded-none border-foreground bg-card"} /></div> }
function TextField({ name, label, value }: { name: string; label: string; value: string }) { return <div><Label className="operator-label">{label}</Label><Input name={name} required defaultValue={value} className="rounded-none border-foreground bg-card" /></div> }
function SelectField({ name, label, value, options, dark }: { name: string; label: string; value: string; options: string[]; dark?: boolean }) { return <div><Label className={`operator-label ${dark ? "text-background/60" : ""}`}>{label}</Label><select name={name} defaultValue={value} className={`h-10 w-full border px-3 text-sm ${dark ? "border-background/40 bg-foreground text-background" : "border-foreground bg-card"}`}>{options.map((option) => <option key={option} value={option}>{option.replaceAll("_", " ")}</option>)}</select></div> }
