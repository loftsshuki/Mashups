...
import { ArrowRight, Check, HardDrive, Loader2, RadioTower, RotateCcw, ShieldCheck, Sparkles } from "lucide-react"

import { Button } from "@/components/ui/button"
import { Input } from "@/components/ui/input"
import { Textarea } from "@/components/ui/textarea"
import { readLocalPilotWorkspace, updateLocalPilotWorkspace } from "@/lib/pilot/local-workspace"
import type { PilotIntake as PilotIntakeRecord, PilotWorkspace } from "@/lib/pilot/types"
...
export function PilotIntake({ workspace, demo }: { workspace: PilotWorkspace; demo: boolean }) {
  ...
  const [markets, setMarkets] = useState(draft?.targetMarkets ?? program?.marketCodes.slice(0, 3) ?? [])
  const [niches, setNiches] = useState(draft?.targetNiches ?? allNiches.slice(0, 3))
  const [goals, setGoals] = useState(draft?.objectives ?? objectives.slice(0, 3))
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (demo) {
        const current = readLocalPilotWorkspace()?.intake
        const localIntake: PilotIntakeRecord = {
          id: current?.id ?? `local-intake-${Date.now()}`, campaignName: body.campaignName, artistName: body.artistName, trackTitle: body.trackTitle,
          contactName: body.contactName, contactEmail: body.contactEmail, genre: body.genre, subgenre: body.subgenre,
          releaseStage: body.releaseStage as PilotIntakeRecord["releaseStage"], releaseDate: body.releaseDate, objectives: body.objectives, targetMarkets: body.targetMarkets, targetNiches: body.targetNiches,
          targetCreatorCount: body.targetCreatorCount, creatorBudgetCents: body.creatorBudgetCents, paidMediaBudgetCents: body.paidMediaBudgetCents, masterUrl: body.masterUrl,
          rightsReadiness: body.rightsReadiness as PilotIntakeRecord["rightsReadiness"], status: body.rightsReadiness === "verified" ? "qualified" : "submitted", notes: body.notes,
        }
        updateLocalPilotWorkspace({ intake: localIntake })
      }
      setResult({ text: `Intake ${data.status ?? "submitted"}. ${demo ? "Saved on this device. " : ""}The operator brief is ready for configuration.`, next: data.next })
  ...
  function restoreDeviceDraft() {
    const saved = readLocalPilotWorkspace()?.intake
    if (!saved) { setResult({ text: "No pilot intake is saved on this device.", error: true }); return }
    setDraft(saved); setMarkets(saved.targetMarkets); setNiches(saved.targetNiches); setGoals(saved.objectives); setFormVersion((version) => version + 1)
    setResult({ text: `Restored ${saved.artistName} / ${saved.trackTitle} from this device.` })
  }

  return <main className="pilot-shell pb-28 pt-[68px]">
    <div className="border-b border-foreground bg-secondary px-4 py-2"><div className="mx-auto flex max-w-[1500px] justify-between gap-4 font-mono text-[10px] font-bold uppercase tracking-[0.16em]"><span>Founding House / Intake 001</span><span>{demo ? "Device-local / no server write" : "Live application"}</span></div></div>
    <section className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
      ...
      <form key={formVersion} onSubmit={submit} className="border-x border-b border-foreground" data-testid="pilot-intake-form">
        <FormSection number="01" title="The release" copy="Give operators enough truth to reject weak-fit campaigns early.">
          <div className="grid gap-5 sm:grid-cols-2"><Field label="Campaign name"><Input name="campaignName" required defaultValue={draft?.campaignName ?? ""} placeholder="Track / founding pilot" /></Field><Field label="Artist"><Input name="artistName" required defaultValue={draft?.artistName ?? ""} /></Field><Field label="Track title"><Input name="trackTitle" required defaultValue={draft?.trackTitle ?? ""} /></Field><Field label="Electronic subgenre"><Input name="subgenre" required defaultValue={draft?.subgenre ?? "House"} placeholder="Melodic house, techno, garage..." /></Field><Field label="Release stage"><NativeSelect name="releaseStage" defaultValue={draft?.releaseStage ?? "unreleased"}><option value="unreleased">Unreleased</option><option value="released">Released</option><option value="catalog">Catalog</option></NativeSelect></Field><Field label="Release date"><Input name="releaseDate" type="date" defaultValue={draft?.releaseDate ?? ""} /></Field></div>
        </FormSection>
        ...
        <FormSection number="03" title="The test" copy="Scope measurable work, budgets, and the decision this campaign needs to make.">
          ...
          <div className="mt-6 grid gap-5 sm:grid-cols-3"><Field label="Creator target"><Input name="targetCreatorCount" type="number" min={10} defaultValue={draft?.targetCreatorCount ?? program.creatorTarget} /></Field><Field label="Creator budget / USD"><Input name="creatorBudget" type="number" min={0} step={50} defaultValue={(draft?.creatorBudgetCents ?? 250000) / 100} /></Field><Field label="Paid test budget / USD"><Input name="paidMediaBudget" type="number" min={0} step={50} defaultValue={(draft?.paidMediaBudgetCents ?? 500000) / 100} /></Field></div>
        </FormSection>
        <FormSection number="04" title="Rights and owner" copy="No track enters the network without a responsible rights contact.">
          <div className="grid gap-5 sm:grid-cols-2"><Field label="Contact name"><Input name="contactName" required defaultValue={draft?.contactName ?? ""} /></Field><Field label="Contact email"><Input name="contactEmail" type="email" required defaultValue={draft?.contactEmail ?? ""} /></Field><Field label="Master file URL"><Input name="masterUrl" type="url" defaultValue={draft?.masterUrl ?? ""} placeholder="https://..." /></Field><Field label="Master authority"><NativeSelect name="rightsReadiness" defaultValue={draft?.rightsReadiness ?? "review"}><option value="unverified">Not yet verified</option><option value="review">Ready for review</option><option value="verified">Authority verified</option></NativeSelect></Field></div><Field label="Operator notes" className="mt-5"><Textarea name="notes" defaultValue={draft?.notes ?? ""} className="min-h-28 rounded-none border-foreground bg-card" /></Field>
        </FormSection>
        <div className="grid gap-6 bg-secondary p-5 sm:p-8 lg:grid-cols-[1fr_auto] lg:items-center"><div><p className="flex items-center gap-2 font-semibold"><ShieldCheck className="size-5" />A submission creates an operating brief, not a performance promise.</p><p className="mt-2 text-sm text-muted-foreground">Device-local mode keeps contact data in this browser only. Do not use it on a shared device.</p>{result ? <p role={result.error ? "alert" : "status"} data-testid="pilot-result" className={`mt-4 border p-3 text-sm ${result.error ? "border-destructive text-destructive" : "border-foreground bg-card"}`}>{result.text}{result.next ? <Link className="ml-2 font-semibold underline" href={result.next}>Open operator setup</Link> : null}</p> : null}</div><div className="flex flex-wrap gap-3"><Button type="button" variant="outline" onClick={restoreDeviceDraft} data-testid="restore-pilot"><RotateCcw />Restore device</Button><Button type="submit" size="lg" disabled={pending || !markets.length || !niches.length || !goals.length} data-testid="submit-pilot">{pending ? <Loader2 className="animate-spin" /> : <HardDrive />}Save intake<ArrowRight /></Button></div></div>
      </form>
    </section>
  </main>
}
...
function Field({ label, children, className = "" }: { label: string; children: React.ReactNode; className?: string }) { return <label className={`block ${className}`}><span className="mb-2 block font-mono text-[10px] font-semibold uppercase tracking-widest">{label}</span>{children}</label> }
