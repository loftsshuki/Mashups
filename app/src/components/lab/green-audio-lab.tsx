  ...
  return <main className="min-h-screen bg-background pb-28 pt-28">
    <section className="mx-auto max-w-[1500px] px-4 sm:px-6 lg:px-8">
      ...
      <header className="mt-7 grid gap-8 border-y border-foreground py-10 lg:grid-cols-12 lg:items-end">
        <div className="lg:col-span-8"><p className="signal-label">Audio systems / arrangement rules</p><h1 className="display-type mt-6 text-[clamp(4rem,10vw,9rem)] leading-[0.78]">CHECK THE<br /><span className="text-primary">ARRANGEMENT.</span></h1></div>
        <div className="lg:col-span-4"><p className="text-lg leading-relaxed text-muted-foreground">These scores check catalog metadata and arrangement rules. They do not measure rendered audio quality or listener preference. Renders below use original synthesis.</p><div className="mt-6 flex flex-wrap gap-3"><Button asChild><Link href="/reports/listening-bench/index.html" prefetch={false}>Open listening bench</Link></Button><Button variant="outline" asChild><a href="/reports/audio-quality-benchmark.html" target="_blank" rel="noreferrer"><ExternalLink />View arrangement checks</a></Button></div></div>
      </header>
      ...
      <div className="grid grid-cols-2 border-x border-b border-foreground md:grid-cols-4">
        ...
        <Metric label="Rule checks passed" value={`${SUMMARY.passRate}%`} signal />
      ...
      <div className="mt-8 grid gap-6 xl:grid-cols-[0.9fr_1.1fr]">
        <section className="border border-foreground bg-card p-5 sm:p-7">
          ...
          <div className="mt-6 grid grid-cols-2 gap-px border border-foreground bg-foreground sm:grid-cols-5"><Score label="Timing rule" value={selected.timingScore} /><Score label="Harmony rule" value={selected.harmonicScore} /><Score label="Phrase rule" value={selected.phraseScore} /><Score label="Overlap rule" value={selected.collisionScore} /><Score label="Candidate rank" value={selected.shareWorthiness} /></div>
