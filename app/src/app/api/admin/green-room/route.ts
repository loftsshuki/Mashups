...
export async function GET() {
  ...
  const [tracks, jobs] = await Promise.all([
    ...
    ctx.admin.from("green_processing_jobs").select("id,track_id,project_id,job_type,status,provider,attempt_count,max_attempts,error_code,lease_expires_at,created_at").order("created_at", { ascending: false }).limit(100),
  ])
...
export async function POST(request: Request) {
  ...
  if (input.action === "queue_processing") {
    ...
    if (!plan.ready) return NextResponse.json({ error: "Audio analysis processor is not configured; no jobs were queued.", missing: plan.missing.map((job) => `${job.jobType}:${job.provider}`) }, { status: 503 })
    const { data: master } = await ctx.admin.from("green_track_assets")
      .select("id,sha256,blob_pathname").eq("track_id", input.trackId).eq("asset_kind", "master")
      .eq("access_level", "private").is("quarantined_at", null).order("created_at", { ascending: false }).limit(1).maybeSingle()
    if (!master?.id || typeof master.sha256 !== "string" || !/^[a-f0-9]{64}$/i.test(master.sha256)) {
      return NextResponse.json({ error: "Master provenance is missing. Re-intake or hash the authorized master before processing." }, { status: 409 })
    }
    const { error } = await ctx.admin.from("green_processing_jobs").insert(plan.jobs.map((job) => ({
      track_id: input.trackId, job_type: job.jobType, provider: job.provider,
      source_asset_id: master.id, source_sha256: master.sha256.toLowerCase(),
      input: { assetPathname: master.blob_pathname, provenanceVersion: 1 },
    })))
    ...
    return NextResponse.json({ ok: !updated.error, queued: plan.jobs.map((job) => `${job.jobType}:${job.provider}`), skipped: plan.missing.map((job) => `${job.jobType}:${job.provider}`) }, { status: updated.error ? 500 : 200 })
  }
