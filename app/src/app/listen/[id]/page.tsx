...
export default function ListenPage(){
 ...
 const [message,setMessage]=useState("Loading publication…"),[shareMessage,setShareMessage]=useState(""),[safetyBusy,setSafetyBusy]=useState(false)
 useEffect(()=>{
 ...
 async function safety(action:"report"|"block"){if(safetyBusy)return;setSafetyBusy(true);try{const response=await fetch("/api/green/safety",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify(action==="report"?{action,publicationId:id,category:"other",details:"Reported from the public listening page."}:{action,publicationId:id})});const body=await response.json();if(response.status===401){setShareMessage("Sign in to report or block a creator.");return}if(!response.ok)throw new Error(body.error??"Safety action could not be confirmed.");setShareMessage(action==="report"?"Report received for review.":"Creator blocked for your account.")}catch(error){setShareMessage(error instanceof Error?error.message:"Safety action could not be confirmed.")}finally{setSafetyBusy(false)}}
