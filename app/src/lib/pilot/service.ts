...
import { getGreenPilotMetrics, unavailableGreenMetrics } from "@/lib/green-room/service"
...
export async function getReadinessSnapshot(userId: string | null, demo = false): Promise<ReadinessSnapshot> {
  ...
  const greenMetrics = greenSchemaReady ? await getGreenPilotMetrics() : unavailableGreenMetrics()
  const retention = greenMetrics.d30RetentionRate === null ? "D30 not yet measurable" : `${Math.round(greenMetrics.d30RetentionRate * 100)}% D30 (${greenMetrics.d30Eligible} eligible)`
  const items: ReadinessItem[] = [
    ...
    { key: "green-schema", category: "product", label: "Green Room rights and render ledger", status: greenSchemaReady && greenMetrics.available ? "ready" : "attention", required: true, detail: greenSchemaReady && greenMetrics.available ? "The catalog ledger and foundation metrics respond. Publication requires reviewed rights, passing audio, and independent listening reviews." : "Apply migrations 024 and 025 to the connected database." },
    { key: "green-funnel", category: "pilot", label: "Green Room activation funnel", status: greenMetrics.available && greenMetrics.sessions >= 100 && greenMetrics.renderCompletionRate >= 0.6 ? "ready" : "attention", required: false, detail: greenMetrics.available ? `${greenMetrics.sessions} sessions / ${Math.round(greenMetrics.renderCompletionRate * 100)}% render completion / ${Math.round(greenMetrics.keepRate * 100)}% keep / ${Math.round(greenMetrics.shareRate * 100)}% share start / ${retention}. iOS and Android remain in the first release programme.` : "Analytics are unavailable. Restore database access and apply migration 025 before assessing activation or retention." },
