...
export async function studioSnapshot(admin: Admin, creatorId: string, id: string): Promise<StudioSnapshot | null> {
  ...
  const [candidates,jobs,owners]=await Promise.all([
    admin.from("green_render_candidates").select("id,arrangement,duration_seconds,quality_status,metrics").eq("project_id",id).order("arrangement"),
    admin.from("green_processing_jobs").select("id,status,error_code,error_message,attempt_count,max_attempts,lease_expires_at").eq("project_id",id).order("created_at",{ascending:false}).order("id",{ascending:false}).limit(1).maybeSingle(),
  ...
  return {
    id:p.id,title:p.title,revision:p.revision,status:p.status,selectedCandidateId:p.selected_candidate_id,parentProjectId:p.parent_project_id,
    candidates:(candidates.data??[]).map(c=>{
      const metrics=(c.metrics && typeof c.metrics==="object" ? c.metrics : {}) as Record<string,unknown>
      return {
        id:c.id,arrangement:c.arrangement,durationSeconds:Number(c.duration_seconds),qualityStatus:c.quality_status,
        ...
        integratedLufs:typeof metrics.integratedLufs==="number"?metrics.integratedLufs:null,
        truePeakDb:typeof metrics.truePeakDb==="number"?metrics.truePeakDb:null,
        technicalPass:typeof metrics.technicalPass==="boolean"?metrics.technicalPass:null,
        renderer:typeof metrics.renderer==="string"?metrics.renderer:null,
      }
    }),
    job:jobs.data ? {
