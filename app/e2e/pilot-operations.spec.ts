...
test("founding intake hands a qualified campaign to the operator", async ({ page }) => {
  ...
  await expect(page.getByText("Labeled simulation / no write")).toBeVisible()
  await page.getByTestId("submit-pilot").click()
  await expect(page.getByTestId("pilot-result")).toContainText("operator brief is ready")
...
test("outreach pipeline advances targets and exports a clean CSV", async ({ page, request }) => {
  ...
  await expect(page.getByText("Fictional pipeline / simulation")).toBeVisible()
